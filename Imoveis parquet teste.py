...
#Logo SH
st.sidebar.image("Ativos/Logo-Escura.png", use_container_width=True)
...
#Setando o dataframe
df_imoveis = pd.read_parquet("Ativos/teste.parquet")
...
def padrao_imovel(valor):
    if valor <= 600000:
        return 'Baixo Padrão'
    elif 600000 < valor <= 1500000:
        return 'Médio Padrão'
    elif 1500000 < valor <= 3500000:
        return 'Alto Padrão'
    else:
...
data_delta_30 = data_maxima - timedelta(days=31)
data_delta_60 = data_maxima - timedelta(days=61)
...
tabela_equipes = tabela_equipes(df_imoveis_ativos)
...
st.sidebar.title("Dashboard - Imóveis Ativos SH Prime")
...
seleção_data_cadastro = st.sidebar.date_input("Data do Cadastro", [datetime.datetime(2016, 1, 1), data_maxima],data_minima, data_hoje, format="DD/MM/YYYY")
...
# 2) FILTRO: EQUIPE OU AGÊNCIA
if seleção_agencia_ou_equipes and seleção_equipe != "Sem Equipe Vinculada":
    tabela_resultados = tabela_resultados[
        tabela_resultados["Equipes do Imovel"].str.contains(seleção_equipe, case=False, na=False)
    ]
elif seleção_agencia_ou_equipes:  # se está ligado, mas == "Sem Equipe Vinculada"
    tabela_resultados = tabela_resultados[
        tabela_resultados["Equipes do Imovel"].isna() |
        tabela_resultados["Equipes do Imovel"].str.strip().eq("")
    ]
elif seleção_agencia != "Todas Agências":  # se não está ligado e != "Todas Agências"
    tabela_resultados = tabela_resultados[
        tabela_resultados["Equipes do Imovel"].str.contains(regex, case=False, na=False)
    ]
...
if seleção_agencia_ou_equipes:
    st.header(f"Imóveis ativos equipe {seleção_equipe}")
    st.subheader(f"Total de imóveis: {total_imoveis}")
else:
    coluna1, coluna2 = st.columns([1, 1])
    with coluna1:
        st.header(f"Imóveis ativos SH {seleção_agencia}")
        st.subheader(f"Total de imóveis: {total_imoveis}")
        seleção_ultimos_30_dias = st.toggle("**últimos 30 dias**")

# 4) EXIBIÇÃO DE MÉTRICAS (SOMENTE QUANDO Botão Equipe OFF)
if not seleção_agencia_ou_equipes:
    colunas = st.columns(13)
    equipes = [
        "Azor", "Falcão", "Carcará", "Faisão", "Gavião",
        "Albatroz", "Fênix", "Sabiá", "Canário", "Wolf",
        "Lion", "Gold", "Infinity"
    ]
    captações = {}
    captações_ultimo_mes = {}
    captações_mes_retrasado = {}
    classificar_atualizacaoaptações_mes_retrasado = {}  # Mantida para não alterar variáveis
    meta_cap_mensal = 20

    # Processa captações de todas as equipes
    for equipe in equipes:
        captações[equipe] = captações_equipe(equipe, data_minima, data_maxima)
        captações_ultimo_mes[equipe] = captações_equipe_último_mês(equipe, captações[equipe])

    # Exibição condicional dos últimos 30 dias
    if not seleção_ultimos_30_dias:
        for i, equipe in enumerate(equipes):
            delta_color = "off" if captações_ultimo_mes[equipe] < meta_cap_mensal else "normal"
            colunas[i].metric(f"**{equipe}**", captações[equipe], captações_ultimo_mes[equipe], delta_color=delta_color)
    else:
        for equipe in equipes:
...
if len(captações_mensais_venda)>1:
    ...
    fig.update_layout(
    title='Quantidade de imóveis por data de cadastro',
    xaxis_title='Mês',
...
with coluna1:
    ...
    annotations=[
            dict(
                text="Baixo Padrão: Até R$600.000.<br>"
                     "Médio Padrão: R$600.000 à R$1.500.000.<br>"
                     "Alto Padrão: R$1.500.000 à R$3.000.000.<br>"
                     "Altíssimo Padrão: Acima de R$3.000.000.<br>",
                x=0,
                y=0.0,  # Acima do gráfico
                xref="paper", yref="paper",align="left",
                showarrow=False,
                font=dict(size=13, color="black")  # Cor cinza para subtexto
            )
        ],
    legend=dict(
